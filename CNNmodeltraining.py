...
def data_x_y_preprocess(datapath):
    ...
    for root,dirs,files in os.walk(datapath):   
        for f in files:
            ...
            plt.imshow(1-np.array(img)/255, cmap='binary')
            plt.title ("original")
            plt.show()
            """
            # img=(1-np.array(img)/255).reshape(1,28,28)
...
# show the data type
print('data_train_x_image:',data_train_x.shape)
print('data_train_y_label',data_train_y_label.shape)

# show the amount of the data
print ('train data = ',len(data_train_x))
print ('test data = ',len(data_test_x))
...
model = Sequential()
# convolutional layer # 建立卷積層，filter=32，即output space深度，Kernel Size=3*3,activation function採用relu
...
model.compile(optimizer="adam",
              ...
              metrics=['accuracy'])
# checkpoint
filepath="10epoch_17_weights.best.hdf5"
...
# save model
model.save('CNN_model_10epoch_17.h5')
